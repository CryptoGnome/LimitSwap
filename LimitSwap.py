...
def load_tokens_file(tokens_path, load_message=True):
    ...
    for token in tokens:
        ...
        token['TRAILING_STOP_LOSS'] = Decimal(token['TRAILING_STOP_LOSS'])
        ...
        while True:
            ...
            for token in tokens:
                ...
                if token['ENABLED'] == 'true':
                    ...
                    # Setting trailing stop boss price if price goes up
                    if token['_QUOTE'] > Decimal(token['SELLPRICEINBASE']) and Decimal(token['TRAILING_STOP_LOSS']) != 0 and token['_TOKEN_BALANCE'] > 0:
                        # We put the price we want to sell in
                        new_sell_price = token['_QUOTE'] * (1 - Decimal(token['TRAILING_STOP_LOSS']) / 100)
                        
                        if (new_sell_price > token['_TRAILING_STOP_LOSS_PRICE']):
                            token['_TRAILING_STOP_LOSS_PRICE'] = new_sell_price
                    ...
                    elif token['_QUOTE'] > Decimal(token['_CALCULATED_SELLPRICEINBASE']) and Decimal(token['TRAILING_STOP_LOSS']) == 0 and token['_TOKEN_BALANCE'] > 0:
                        printt_warn("--------------------------------------------------------------")
                        printt_warn("Sell Signal Found =-= Sell Signal Found =-= Sell Signal Found ", write_to_log=True)
                        price_conditions_met = True
                    ...
                    elif Decimal(token['TRAILING_STOP_LOSS']) != 0 and token['_TOKEN_BALANCE'] > 0 and token['_QUOTE'] < float(token['_TRAILING_STOP_LOSS_PRICE']) and token['_QUOTE'] > float(token['ANTI_DUMP_PRICE']):
                        printt_warn("--------------------------------------------------------------")
